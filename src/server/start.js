...
import { initializeSchema as initSchema } from './utils/schemaInit.js';
...
async function startServer() {
  try {
    ...
    if (!dbConnected) {
      logger.db.error('Failed to connect to database, starting server anyway');
    } else {
      // Initialize schema
      const schemaInitialized = await initSchema();
      if (!schemaInitialized) {
        logger.db.error('Failed to initialize schema, starting server anyway');
      }
      
      // Initialize MS365 license types schema
      const ms365SchemaInitialized = await initMS365Schema();
      if (!ms365SchemaInitialized) {
        logger.db.error('Failed to initialize MS365 license types schema, starting server anyway');
      }
    }
    
    // Start the server
    app.listen(PORT, () => {
      logger.db.info(`Server running on port ${PORT}`);
    });
  } catch (error) {
    logger.db.error('Error starting server:', error);
    process.exit(1);
  }
}
